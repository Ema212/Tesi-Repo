...
::: {#imag_1 .center}
![](../figures/esgen.png){width=40%,align.center}

Figure 2.2.1: A graphical rapresentation of a Cohen's *d*, in a RCT group. [@magnussonCausalTherapistEffects]
:::

We have provided a graphic illustration of an effect size, which represents values distribution when the treatment has no effect (H~0~) and when the effect is 0.41 (H~1~). It is quite intuitive though that neither a graphical representation nor a numerical value clearly informs us of _how much_ of improvement patient have experienced under the alternative hypothesis. This example introduces the main matter discussed in this thesis that we will detail in the following section, that is interpretation of effect size.
...
A graphical rapresentation of the three effect sizes is reported at figure 2.2.1, 2.2.2 and 2.2.3 respectively. These sets of distributions derive from a hypothetical comparison between a treatment group and a control group in a pre-post design. The black curves always represent the distribution under the null hypothesis (H~0~), which assumes that the intervention has no effect (i.e., the mean change in the treatment group is equal to that in the control group) corresponding to an effect size of *d* = 0. The blue curves always  represent the alternative hypothesis (H~1~), assuming a true standardized effect size of of *d* = 0.2, *d* = 0.5, *d* = 0.8, meaning the treatment group improves, on average, by 0.2, 0.5 and 0.8 standard deviations more than the control group, respectively. The distance between each pair of distributions reflects the assumed effect magnitudes. 

::: {#imag_1 .center}
![](../figures/02es.jpg){width=40%,align.center}

Figure 2.2.1: A graphical rapresentation of 0.2 Cohen's *d* [@magnussonCausalTherapistEffects].
:::

::: {#imag_1 .center}
![](../figures/05es.jpg){width=40%,align.center}

Figure 2.2.2: A graphical rapresentation of 0.5 Cohen's *d* [@magnussonCausalTherapistEffects].
:::

::: {#imag_1 .center}
![](../figures/08es.jpg){width=40%,align.center}

Figure 2.2.3: A graphical rapresentation of 0.8 Cohen's *d* [@magnussonCausalTherapistEffects].
:::

While Cohen himself intended these benchmarks as a last resource reference, psychology research has adopted them as standards. Unfortunately, applying these benchmarks without considering the context leads to uninformative conclusions [@funder2019evaluating]. Effect sizes require different interpretations depending on the field of interest, the specific content, and the research methods used, as in a given field a small effect size can be extremely relevant in practice[@altoe2020enhancing].
...
\textbf{} & \textbf{Plausible effect size}      & \textbf{SESOI} \\
\hline
\textbf{Pros}   & • Evidence based              & • Practically informative\\
\hline  
\textbf{Cons}   & • Practically uninfromative   & • Strongly relies on subjectivity\\
                & • Requires high-quality meta-analysis & • Requires attentive esteems\\
\hline
